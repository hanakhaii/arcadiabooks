          ...
          <main>
            ...
            <section class="all-books-time">
              ...
              <div class="books-coulum">
                <!-- for -book div -->
                 <?php foreach($perpus->loantime($email) as $l): 
                    $loan_date = strtotime($l['loan_date']);
                    $estimated_return = strtotime('+7 days', $loan_date);
                    $today = time();

                    if ($today <= $estimated_return) {
                      // Belum lewat tenggat, hitung sisa hari
                      $days_left = ceil(($estimated_return - $today) / 86400);
                      $tenggat_text = "Tenggat Waktu: {$days_left} hari lagi";
                    } else {
                      // Sudah lewat tenggat, hitung keterlambatan
                      $late_days = floor(($today - $estimated_return) / 86400);
                      $tenggat_text = "<span style='color:red;'>Sudah lewat {$late_days} hari dari tenggat</span>";
                    }
                  ?>
